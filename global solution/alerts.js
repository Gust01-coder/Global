...
class FloodAlertSystem {
    ...
    displayAlerts() {
        ...
        alertsFeed.innerHTML = filteredAlerts.map(alert => `
            <div class="card border-0 shadow-sm mb-3 alert-level-${alert.level}">
                <div class="card-body">
                    <div class="row align-items-center">
                        <div class="col-auto">
                            <div class="alert-icon text-${alert.color}">
                                <i class="fas fa-${alert.icon} fa-2x"></i>
                            </div>
                        </div>
                        <div class="col">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                ...
                                <div class="d-flex gap-2">
                                    <span class="severity-badge bg-${alert.color} text-white">
                                        ${alert.severity}
                                    </span>
                            ...
                            <div class="row text-muted small">
                                ...
                                <div class="col-sm-6">
                                    ...
                                    ${alert.affectedPeople.toLocaleString()} pessoas afetadas
                                </div>
                            </div>
                        </div>
                        <div class="col-auto">
                            <div class="btn-group btn-group-sm">
                                <button class="btn btn-outline-primary" onclick="alertSystem.viewDetails('${alert.id}')">
                                    <i class="fas fa-eye"></i> Detalhes
                                </button>
                                <button class="btn btn-outline-${alert.color}" onclick="alertSystem.shareAlert('${alert.id}')">
        ...
        this.alerts.forEach(alert => {
            if (alert.status === 'active') {
                ...
                const marker = L.marker([alert.coordinates.lat, alert.coordinates.lng], {
                    icon: L.divIcon({
                        html: `<div class="custom-marker text-${alert.color}">
        ...
        historyContainer.innerHTML = recentAlerts.map(alert => `
            <div class="timeline-item">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h6 class="mb-1 fw-bold text-${alert.color}">
                            <i class="fas fa-${alert.icon} me-1"></i>${alert.title}
                        </h6>
                        <p class="mb-1 small">${alert.location}</p>
                        <p class="mb-0 text-muted small">${this.formatTime(alert.timestamp)}</p>
                    </div>
                    <span class="badge bg-${alert.color}">${alert.severity}</span>
                </div>
            </div>
        `).join('');
    }
    ...
    generateRandomAlert() {
        const alertTypes = [
            { level: 1, type: 'info', title: 'Informativo', color: 'success', icon: 'info-circle' },
            { level: 2, type: 'warning', title: 'Atenção', color: 'warning', icon: 'exclamation-triangle' },
            { level: 4, type: 'critical', title: 'Crítico', color: 'danger', icon: 'ban' }
        ];
